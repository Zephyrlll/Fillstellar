...
export class ConversionEngine {
    private activeConversions: Map<string, ConversionQueue> = new Map();
    private facilities: Map<string, ProductionFacility> = new Map();
    ...
    addFacility(facility: ProductionFacility): void {
    ...
    getAllFacilities(): ProductionFacility[] {
    ...
    private getResourceAmount(type: ResourceType, minQuality: QualityTier = QualityTier.POOR): number {
    ...
    private consumeResources(recipe: ConversionRecipe): boolean {
    ...
    private produceResources(recipe: ConversionRecipe, inputQuality: QualityTier, facilityEfficiency: number): void {
    ...
    getWasteStatus(): { amount: number; capacity: number; percentage: number } {
        const wasteType = ResourceType.RADIOACTIVE_WASTE;
        const wasteStorage = gameState.advancedResources?.[wasteType];
    ...
    isFacilityBusy(facilityId: string): boolean {
    ...
    loadState(state: any): void {
        if (state.facilities) {
            ...
            state.facilities.forEach((facility: ProductionFacility) => {
                this.facilities.set(facility.id, facility);
            });
        }
        ...
        if (state.activeConversions) {
            ...
            state.activeConversions.forEach((conv: any) => {
