...
interface ProductionChainView {
    nodes: Map<string, ProductionNode>;
    connections: Array<{ from: string; to: string; amount?: number }>;
}

export class ProductionChainUI {
    private container: HTMLElement | null = null;
    private chainView: ProductionChainView;
    ...
    private buildChainData(): void {
        ...
        resources.forEach(key => {
            const amount = gameState.resources[key as keyof typeof gameState.resources];
            if (amount > 0) {
        ...
        if (gameState.advancedResources) {
            Object.entries(gameState.advancedResources).forEach(([type, storage]) => {
                if (storage.amount > 0) {
                    const metadata = RESOURCE_METADATA[type as ResourceType];
                    if (metadata) {
        ...
        activeConversions.forEach(conversion => {
            ...
            // Add recipe node
            const recipeNode: ProductionNode = {
                id: recipe.id,
                type: 'recipe',
                name: recipe.name,
    ...
    private getResourceTypeFromKey(key: string): ResourceType | null {
        const mapping: { [key: string]: ResourceType } = {
    ...
    // Render node flow
    private renderNodeFlow(node: ProductionNode): string {
        let flow = '';
    ...
    // Update waste status
    private updateWasteStatus(): void {
