...
export function detectCollisions() {
    ...
    gameState.stars.forEach(body1 => {
        if (!body1.userData || body1.userData.isStatic)
            return;
        const nearby = spatialGrid.getNearbyObjects(body1, body1.userData.radius * 3);
        nearby.forEach(body2 => {
            if (body2 === body1 || !body2.userData || body2.userData.isStatic)
                return;
...
export function handleCollision(body1, body2) {
    ...
    if (survivor.userData.type === 'star' && absorbed.userData.type === 'star') {
        ...
        const survivorData = survivor.userData;
        const absorbedData = absorbed.userData;
