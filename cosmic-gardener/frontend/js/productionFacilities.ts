...
import { ProductionFacility } from './resourceSystem.js';
import { gameState } from './state.js';
import { conversionEngine } from './conversionEngine.js';
import { addTimelineLog } from './timeline.js';
import { showMessage } from './ui.js';

export const PRODUCTION_FACILITIES: Record<string, ProductionFacility> = {
...
// Facility effects
export const FACILITY_EFFECTS: Record<string, (level: number) => void> = {
    'waste_storage': (level: number) => {
...
export function canAffordFacility(facilityId: string): boolean {
    ...
    return cost.resources.every(requirement => {
        ...
        if (resourceType in gameState.resources) {
            available = gameState.resources[resourceType as keyof typeof gameState.resources];
        }
...
export function payForFacility(facilityId: string): boolean {
    const cost = FACILITY_COSTS[facilityId];
    if (!cost || !canAffordFacility(facilityId)) return false;
    ...
    cost.resources.forEach(requirement => {
        ...
        if (resourceType in gameState.resources) {
            gameState.resources[resourceType as keyof typeof gameState.resources] -= amount;
        }
...
export function addFacilityToGame(facilityId: string): void {
