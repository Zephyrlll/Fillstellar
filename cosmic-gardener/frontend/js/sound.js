class SoundManager {
    ...
    async createSpaceReverb() {
        if (!this.audioContext || !this.reverb)
            return;
    ...
    // 天体作成サウンド
    createCelestialBodySound(type, position) {
        if (!this.audioContext || !this.effectsGain || !this.initialized)
            return;
        const now = this.audioContext.currentTime;
        switch (type) {
            case 'dust':
    ...
    playDustCreationSound(position) {
    ...
    playAsteroidCreationSound(position) {
        if (!this.audioContext || !this.effectsGain)
            return;
        // 衝突音のシミュレーション
    ...
    playPlanetCreationSound(position) {
        if (!this.audioContext || !this.effectsGain)
            return;
        // 深い共鳴音
        const oscillator1 = this.audioContext.createOscillator();
    ...
    playStarCreationSound(position) {
        if (!this.audioContext || !this.effectsGain)
            return;
        // インターステラー風の壮大な星の誕生音
    ...
    // 生命進化サウンド
    playEvolutionSound(evolutionType, position) {
        if (!this.audioContext || !this.effectsGain || !this.initialized)
            return;
    ...
    // UIサウンド
    playUISound(type) {
        if (!this.audioContext || !this.uiGain || !this.initialized)
            return;
        switch (type) {
            case 'click':
                this.playClickSound();
    ...
    playClickSound() {
        if (!this.audioContext || !this.uiGain)
            return;
        const oscillator = this.audioContext.createOscillator();
        const gain = this.audioContext.createGain();
        oscillator.type = 'sine';
    ...
    // ユーティリティメソッド
    connectWithSpatialAudio(...nodes) {
        if (!this.audioContext || !this.settings.spatialAudio) {
            // 空間オーディオ無効時は通常の接続
            const audioNodes = nodes.filter(n => n && 'connect' in n);
        ...
        // 空間オーディオ有効時
        const position = nodes.find(n => n && 'x' in n);
        const audioNodes = nodes.filter(n => n && 'connect' in n);
    ...
    applySettings() {
        if (!this.masterGain || !this.ambientGain || !this.effectsGain || !this.uiGain)
            return;
        this.masterGain.gain.value = this.settings.muted ? 0 : this.settings.masterVolume;
        this.ambientGain.gain.value = this.settings.ambientVolume;
        this.effectsGain.gain.value = this.settings.effectsVolume;
    ...
    updateListenerPosition(position, orientation) {
        ...
        if (orientation) {
            this.audioContext.listener.setOrientation(orientation.x, orientation.y, orientation.z, 0, 1, 0 // up vector
            );
        }
    }
    // クリーンアップ
