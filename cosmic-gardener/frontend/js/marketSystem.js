...
export class MarketSystem {
    ...
    calculateBaseLimit(resourceType) {
        ...
        if (basePrice >= 100)
            return 50; // Tier 3 resources
        if (basePrice >= 10)
            return 200; // Tier 2 resources  
        if (basePrice >= 1)
            return 1000; // Biomass, thought points
    ...
    updatePrices() {
        Object.entries(this.marketData).forEach(([resourceType, data]) => {
            ...
            data.currentPrice = Math.max(data.basePrice * 0.3, // Minimum 30% of base price
            Math.min(data.basePrice * 3.0, // Maximum 300% of base price
    ...
    sellResource(resourceType, amount) {
        ...
        if (!limitData) {
            return { success: false, message: '市場データが見つかりません' };
        }
