...
import { QualityTier, RESOURCE_METADATA, getResourceDisplayName, getQualityName, QUALITY_MULTIPLIERS } from './resourceSystem.js';
import { getAvailableRecipes } from './conversionRecipes.js';
...
export function updateProductionUI(force = false) {
...
function updateAdvancedResourcesDisplay() {
    ...
    advancedResourcesDisplay.querySelectorAll('.advanced-resource-item').forEach(item => {
        ...
        if (quality >= QualityTier.HIGH_QUALITY) {
            resourceParticleSystem.addQualityGlow(item, quality);
        }
    });
}
function updateConversionRecipesList() {
    if (!conversionRecipesList)
        return;
    const availableRecipes = getAvailableRecipes(gameState.discoveredTechnologies, gameState.availableFacilities);
    const html = [];
...
function updateActiveConversionsList() {
    if (!activeConversionsList)
        return;
...
function updateProductionFacilitiesList() {
    if (!productionFacilitiesList)
        return;
