...
class ResourceParticleSystem {
    private particles: ResourceParticle[] = [];
    ...
    createQualityParticles(element: HTMLElement, quality: QualityTier): void {
        if (quality <= QualityTier.STANDARD) return; // Only show particles for high quality+
        ...
        (particleSystem.material as THREE.PointsMaterial).opacity = 0.8;
        ...
        vector.unproject(scene.children[0] as THREE.Camera); // Assuming camera is first child
    ...
    addQualityGlow(element: HTMLElement, quality: QualityTier): void {
        const qualityData = QUALITY_MULTIPLIERS[quality];
