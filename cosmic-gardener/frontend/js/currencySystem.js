...
console.log('🔧 currencySystem.js loaded');
...
export class CurrencyManager {
    ...
    loadState(state) {
        if (state.currencies) {
            gameState.currencies = { ...state.currencies };
        }
    }
}
...
export function formatCurrency(amount, currencyType) {
    const definition = CURRENCY_DEFINITIONS[currencyType];
    if (!definition) return amount.toString();
...
// Export to window for debugging and cross-module access
