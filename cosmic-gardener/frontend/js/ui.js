...
export function updateUI() {
    const { gameYear, cosmicDust, energy, organicMatter, biomass, darkMatter, thoughtPoints, dustUpgradeLevel, darkMatterConverterLevel, researchEnhancedDust, researchAdvancedEnergy, unlockedCelestialBodies, currentDustRate, cachedTotalPopulation } = gameState;
    ...
    if (previousUIValues.gameYear !== currentGameYear) {
        if (ui.gameYear)
            ui.gameYear.textContent = String(currentGameYear);
    ...
    if (previousUIValues.energy !== currentEnergy) {
        if (ui.energy)
            ui.energy.textContent = String(currentEnergy);
        previousUIValues.energy = currentEnergy;
    }
    if (previousUIValues.darkMatter !== currentDarkMatter) {
        if (ui.darkMatter)
            ui.darkMatter.textContent = String(currentDarkMatter);
    ...
    if (previousUIValues.organicMatter !== currentOrganicMatter) {
        if (ui.organicMatter)
            ui.organicMatter.textContent = String(currentOrganicMatter);
    ...
    if (previousUIValues.biomass !== currentBiomass) {
        if (ui.biomass)
            ui.biomass.textContent = String(currentBiomass);
    ...
    if (previousUIValues.thoughtPoints !== currentThoughtPoints) {
        if (ui.thoughtPoints)
            ui.thoughtPoints.textContent = String(currentThoughtPoints);
    ...
    if (previousUIValues.dustRate !== currentDustRateStr) {
        if (ui.dustRate)
            ui.dustRate.textContent = currentDustRateStr;
        previousUIValues.dustRate = currentDustRateStr;
    }
    if (ui.upgradeDustButton)
        ui.upgradeDustButton.disabled = currentEnergy < nextDustUpgradeCost;
    if (ui.upgradeDarkMatterConverterButton)
        ui.upgradeDarkMatterConverterButton.disabled = currentEnergy < nextConverterCost;
    if (previousUIValues.dustUpgradeLevel !== dustUpgradeLevel) {
        if (ui.dustUpgradeLevel)
            ui.dustUpgradeLevel.textContent = String(dustUpgradeLevel);
        previousUIValues.dustUpgradeLevel = dustUpgradeLevel;
    }
    if (previousUIValues.dustUpgradeCost !== nextDustUpgradeCost) {
        if (ui.dustUpgradeCost)
            ui.dustUpgradeCost.textContent = String(nextDustUpgradeCost);
    ...
    if (previousUIValues.darkMatterConverterLevel !== darkMatterConverterLevel) {
        if (ui.darkMatterConverterLevel)
            ui.darkMatterConverterLevel.textContent = String(darkMatterConverterLevel);
    ...
    if (previousUIValues.darkMatterConverterCost !== nextConverterCost) {
        if (ui.darkMatterConverterCost)
            ui.darkMatterConverterCost.textContent = String(nextConverterCost);
        previousUIValues.darkMatterConverterCost = nextConverterCost;
    }
    if (ui.researchEnhancedDustButton)
        ui.researchEnhancedDustButton.disabled = researchEnhancedDust || currentDarkMatter < 1;
    if (ui.researchAdvancedEnergyButton)
        ui.researchAdvancedEnergyButton.disabled = researchAdvancedEnergy || currentDarkMatter < 2;
    if (ui.researchMoonButton)
        ui.researchMoonButton.disabled = unlockedCelestialBodies.moon || currentDarkMatter < 1;
    if (ui.researchDwarfPlanetButton)
        ui.researchDwarfPlanetButton.disabled = unlockedCelestialBodies.dwarfPlanet || currentDarkMatter < 2;
    if (ui.researchPlanetButton)
        ui.researchPlanetButton.disabled = unlockedCelestialBodies.planet || currentDarkMatter < 3;
    const researchStarCost = 5;
    if (ui.researchStarButton)
        ui.researchStarButton.disabled = unlockedCelestialBodies.star || currentDarkMatter < researchStarCost;
    if (previousUIValues.researchStates.enhancedDust !== researchEnhancedDust) {
        if (ui.researchEnhancedDustStatus)
            ui.researchEnhancedDustStatus.textContent = researchEnhancedDust ? '完了' : '未完了';
    ...
    if (previousUIValues.researchStates.advancedEnergy !== researchAdvancedEnergy) {
        if (ui.researchAdvancedEnergyStatus)
            ui.researchAdvancedEnergyStatus.textContent = researchAdvancedEnergy ? '完了' : '未完了';
    ...
    if (previousUIValues.unlockedBodies.moon !== unlockedCelestialBodies.moon) {
        if (ui.researchMoonStatus)
            ui.researchMoonStatus.textContent = unlockedCelestialBodies.moon ? '完了' : '未完了';
        if (ui.createMoonButton)
            ui.createMoonButton.style.display = unlockedCelestialBodies.moon ? 'inline-block' : 'none';
    ...
    if (previousUIValues.unlockedBodies.dwarfPlanet !== unlockedCelestialBodies.dwarfPlanet) {
        if (ui.researchDwarfPlanetStatus)
            ui.researchDwarfPlanetStatus.textContent = unlockedCelestialBodies.dwarfPlanet ? '完了' : '未完了';
        if (ui.createDwarfPlanetButton)
            ui.createDwarfPlanetButton.style.display = unlockedCelestialBodies.dwarfPlanet ? 'inline-block' : 'none';
    ...
    if (previousUIValues.unlockedBodies.planet !== unlockedCelestialBodies.planet) {
        if (ui.researchPlanetStatus)
            ui.researchPlanetStatus.textContent = unlockedCelestialBodies.planet ? '完了' : '未完了';
        if (ui.createPlanetButton)
            ui.createPlanetButton.style.display = unlockedCelestialBodies.planet ? 'inline-block' : 'none';
    ...
    if (previousUIValues.unlockedBodies.star !== unlockedCelestialBodies.star) {
        if (ui.researchStarStatus)
            ui.researchStarStatus.textContent = unlockedCelestialBodies.star ? '完了' : '未完了';
        if (ui.researchStarCost)
            ui.researchStarCost.textContent = String(researchStarCost);
...
export function switchTab(activeTab) {
    ...
    if (ui.productionScreen)
        ui.productionScreen.classList.add('hidden-screen');
    if (ui.optionsScreen)
        ui.optionsScreen.classList.add('hidden-screen');
    if (ui.starManagementScreen)
    ...
    if (ui.productionTabButton)
        ui.productionTabButton.classList.remove('active-tab');
    ...
    if (activeTab === 'game') {
        ...
        if (ui.productionScreen)
            ui.productionScreen.classList.remove('hidden-screen');
        if (ui.productionTabButton)
            ui.productionTabButton.classList.add('active-tab');
...
export function debouncedUpdateGalaxyMap() {
    ...
    const significantChange = previousGalaxyMapState.starCount !== currentStarCount ||
...
export function updateGraphicsUI() {
...
export function updateDeviceInfoDisplay() {
...
export function syncUIWithGraphicsState() {
...
export function getGraphicsSettingValue(settingName) {
...
export function setGraphicsSettingValue(settingName, value) {
