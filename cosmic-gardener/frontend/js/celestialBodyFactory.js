...
import { CelestialCreationError, Ok, Err } from './types/celestial.js';
...
export class CelestialBodyFactory {
    ...
    static create(type, config) {
        ...
        try {
            ...
            if (!validation.isValid) {
                return Err(new CelestialCreationError('VALIDATION_ERROR', 'Configuration validation failed', { errors: validation.errors }));
            }
            // 天体作成
            const body = factory.createCelestialBody(type, config);
            if (!body) {
                return Err(new CelestialCreationError('CREATION_FAILED', `Failed to create celestial body of type: ${type}`));
            }
            ...
            return Err(new CelestialCreationError('UNEXPECTED_ERROR', error instanceof Error ? error.message : 'Unknown error occurred', error));
        }
    }
    // パラメータ検証
    ...
    // 実際の天体作成処理
    createCelestialBody(type, config) {
        let body;
    ...
    // 天体タイプ別のデータ作成
    createSpecificCelestialData(type, config) {
        switch (type) {
            case 'star':
    ...
    createStarData() {
        ...
        starData.lifespan = this.calculateStarLifespan(starData);
        const gameMass = starData.mass * 1000;
        const radius = Math.max(Math.cbrt(gameMass) * 8.0, 15.0);
        return { mass: gameMass, radius, data: starData };
    }
    // 恒星の寿命計算
    calculateStarLifespan(starData) {
    ...
    // 惑星データ作成
    createPlanetData(parent) {
        if (!parent || !parent.userData)
            return null;
        const parentMass = parent.userData.mass;
    ...
    createStarObject(params) {
        ...
        body.userData.originalRadius = params.radius;
        body.userData.materialType = 'star';
        return body;
    }
    // 惑星オブジェクト作成
    createPlanetObject(params) {
        const planetData = params.specificData;
        const maps = this.createRealisticPlanetMaps(planetData.subType, parseFloat(planetData.water), parseFloat(planetData.atmosphere));
        ...
        planetMesh.userData.originalRadius = params.radius;
    ...
    createDwarfPlanetObject(params) {
    ...
    // ヘルパーメソッド
    addOutline(body, radius, color, opacity) {
    ...
    addAtmosphere(body, planetData, radius) {
        let atmosphereColor = 0xffffff;
        if (planetData.subType === 'neptune_like')
            atmosphereColor = 0x4169E1;
        else if (planetData.subType === 'terran' || planetData.subType === 'ocean_world')
    ...
    addAccretionDisk(blackHoleGroup, radius) {
    ...
    createRealisticAsteroid(radius) {
    ...
    createRealisticPlanetMaps(subType, water, atmosphere) {
...
export function createPlanet(parentStar) {
    const factory = CelestialBodyFactory.getInstance();
